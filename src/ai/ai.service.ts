...
export class AiService {
  ...
  public async askAI(companyForm: CompanyFormDto): Promise<string> {
    const allSkills = Object.values(SkillList.skills).flat().join(', ');

    let requiredSkills = companyForm.requiredSkills
      ? companyForm.requiredSkills.join(', ')
      ...
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [
          ...
          },
          {
            ...
            }, souhaite réaliser un projet dont la description est : "${
              companyForm.projectDescription
            }". L'entreprise a un budget de ${
              companyForm.budget
            } euros et souhaite que le projet soit terminé en ${
              companyForm.projectDurationWeeks ?? 'non précisé'
            } semaines. Les compétences requises pour le projet sont les suivantes : ${requiredSkills}. Les contraintes techniques du projet sont : ${
              companyForm.technicalRequirements ?? 'aucune spécifiée'
            }. Le public cible est : ${
              companyForm.targetAudience ?? 'non précisé'
            }. Le contenu nécessaire est ${
              companyForm.contentAvailability ?? 'non précisé'
            }. Le projet ${
              companyForm.requiresMarketingOrSEO
                ? 'nécessite'
                : 'ne nécessite pas'
            } une stratégie de marketing ou SEO et ${
              companyForm.hasSocialMediaStrategy ? 'inclut' : 'n\'inclut pas'
            } une gestion des réseaux sociaux ou une stratégie d'influence.
