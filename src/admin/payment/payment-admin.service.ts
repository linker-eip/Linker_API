...
export class PaymentAdminService {
    ...
    async getStudentPayments() : Promise<StudentPaymentResponseDto[]> {
        ...
        const studentPaymentResponseDtos = studentPayments.map(async (studentPayment) => {
            const student = await this.userAdminService.findOneStudentById(studentPayment.studentId);
            if (!student) {
                throw new NotFoundException('Student not found');
            }
            const mission = await this.missionAdminService.findMissionById(studentPayment.missionId);
    ...
    async updateStudentPayment(body : UpdateStudentPaymentDto) : Promise<StudentPaymentResponseDto> {
        const studentPayment = await this.studentPaymentRepository.findOne({ where: { id: body.id } });
        if (!studentPayment) {
